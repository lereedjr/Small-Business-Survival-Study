...
'''
zip_to_county_sum = zip_to_county.groupby('zip', as_index=False)['res_ratio','bus_ratio','oth_ratio','tot_ratio'].mean()
...
'''
zip_to_county_sum['principalzipcode'] = np.vectorize(makezipstr)(zip_to_county_sum['zip'])
#test new column
print(zip_to_county_sum.head())
...
'''
sos_updated_file = pd.merge(sos_updated_file,zip_to_county_sum, how='left', on='principalzipcode')
